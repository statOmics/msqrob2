...
peptidesFile<-"~/Dropbox/proteomicsShortcourse/pda2019/data/quantification/cptacAvsB_lab3/peptides.txt"
ecols<-MSnbase::grepEcols(peptidesFile, "Intensity ", split = "\t")
pe<-readFeatures(table=peptidesFile,ecol=ecols,fnames=1,name="peptide",sep="\t")
```
...
```{r}
protnames<-rownames(pe[["protein"]])[assay(pe[["protein"]]) %>% is.na %>% `!` %>% rowSums>4]
pe<-pe[protnames,,]
```
...
Extract coefficients for first protein

```{r}
getCoef(rowData(pe[["protein"]])$model[[1]])
```
