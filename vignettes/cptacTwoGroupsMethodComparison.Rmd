...
Proteomic Technology Assessment for Cancer (CPTAC). In this experiment, the
authors spiked the Sigma Universal Protein Standard mixture 1 (UPS1)
containing 48 different human proteins in a protein background of
60 ng/$\mu$L Saccharomyces cerevisiae strain BY4741.
Two different spike-in concentrations were used: 6A (0.25 fmol
UPS1 proteins/$\mu$L) and 6B (0.74 fmol UPS1 proteins/$\mu$L) [5].
We limited ourselves to the data of LTQ-Orbitrap W at site 56.
The data were searched with MaxQuant version 1.5.2.8, and detailed search
settings were described in Goeminne et al. (2016) [1].
...
We can extract the spike-in condition from the raw file name.
...
peptide with zero intensities are missing peptide and should be represent with
a `NA` value rather than `0`.
```{r}
...
`r format(mean(is.na(assay(pe[["peptideRaw"]])))*100,digits=2)`% of all
peptide intensities are missing and for some peptides we do not
even measure a signal in any sample. The missingness is similar across samples.
Note, that we plot the peptide data, so the label protein in the plot
refers to peptides.
...
differences (log FC) in the peptideRaw data seems to be driven by technical
variability. Indeed, the samples do not seem to be clearly 
...
We notice that the leading differences (log FC) in the protein data
is still according to technical variation. On the second dimension; however,
we also observe a clear separation according to the spike-in condition.
Hence, the summarization that accounts for peptide specific effects makes
the effects due to the spike-in condition more prominent!
...
for samples from condition a is '(Intercept). The mean log2 expression
for samples from condition B is '(Intercept)+conditionB'. Hence, the average
log2 fold change between condition b and condition a is modelled using
the parameter 'conditionB'. Hence, we assess the contrast 'conditionB=0'
with our statistical test.
...
Note, that the volcano plot opens up and that the dots are more tight.
...
logFC %>% ggplot(aes(x = method, y = log2FC, fill = ups)) + geom_boxplot() +
