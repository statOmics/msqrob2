...
We will compare different methods based on this two-groups spike in data set.
...
The DE proteins are indeed having a log2 fold change that is close to the real fold change for the spiked proteins.
...
```{r}
pe <- aggregateFeatures(pe, fcol = "Proteins", na.rm = TRUE, name="proteinMedian",fun=matrixStats::colMedians)
...
Ridge regression does not improve the performance in two-group comparisons.
...
In the vignette of a large spike-in study with multiple spike-in conditions we illustrate the benefit of ridge regression.
