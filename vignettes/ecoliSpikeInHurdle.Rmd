...
# Background
In this vignette we assess the performance of the MSqRob Hurdle workflow for differential expression analysis using a publicly available
...
We make and additional summarisation assay where we summarise the peptides to protein values by counting how many peptides are observed for each protein.

We also need to create a column in the rowData of the assay with the number of peptides that could be observed per protein, which we call nPep.

```{r}
pe <- aggregateFeatures(pe, i="peptideNorm", fcol = "Proteins", name="proteinCount", fun=nObsPep)
rowData(pe[["proteinCount"]])$nPep<- rowData(pe[["peptideNorm"]]) %>%
        data.frame(.$Proteins)  %>%
        group_by(Proteins) %>%
...
## Estimation intensity based method
...
And we adopt the hypothesis tests for each contrast and for both hurdle components.

```{r}
pe <- hypothesisTest(object=pe,i="protein",contrast=L)
pe <- hypothesisTest(object=pe,i="proteinCount",contrast=L)
for (j in colnames(L))
{
  sam <- cbind(rowData(pe[["protein"]])[[j]][,1:5],
               rowData(pe[["proteinCount"]])[[j]][,1:5])
  ...
  sam$fisherAdjPval <- p.adjust(sam$fisherPval,"BH")
  rowData(pe[["proteinCount"]])[[paste0("hurdle",j)]] <- sam
}
```
...
for (i in colnames(L))
{
sigNames <- rowData(pe[["proteinCount"]])[[paste0("hurdle",i)]] %>%
    rownames_to_column("protein") %>% filter(fisherAdjPval<0.01) %>% pull(protein)
...
tprFdpHurdle[[i]] <- tprFdp(rowData(pe[["proteinCount"]])[[paste0("hurdle",i)]]$fisherPval,
             rowData(pe[["protein"]])$accession$ecoli,rowData(pe[["proteinCount"]])[[paste0("hurdle",i)]]$fisherAdjPval)
