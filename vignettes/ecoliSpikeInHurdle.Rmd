...
were spiked in at five different concentrations (3%, 4.5%, 6%, 7.5% and 9% wt/wt)
...
In this tutorial, we will use a MaxQuant peptides file from the ecoli spike-in
study that is stored in the msdata package. We use the `Features` package to
import the data.

With the `grepEcols` function we find the columns that contain the
...
cond <- which(strsplit(colnames(pe)[[1]][1], split = "")[[1]] == "a")
colData(pe)$condition <- substr(colnames(pe), cond, cond) %>% unlist %>%  as.factor
```

We calculate how many non zero intensities we have for each peptide.
This is going to be useful for filtering.
...
The ecoli peptides are spiked in.
...
that the data should not be log-transformed prior to using `vsn` normalisation.
...
leading differences (log FC) in the peptide data seems to be driven
by the spike-in condition.
...
With the hurdle approach, we model the protein level expression values using msqrob.
By default the hurdle model estimates the model parameters using robust regression.
By default, this model component is stored in the row data under name "msqrobHurdleIntensity".
The number of peptides that were used to summarize to protein level expression
values are modelled using a quasi-binomial model. This model component is
stored by default in the row data under the name "msqrobHurdleCount".
...
The following chunk extracts the parameter names of the model.
```{r}
...
average log2 fold change (FC) between condition b and condition a is modelled
using the parameter 'conditionb'. Thus, we assess the contrast 'conditionb = 0'
with our statistical test. The same holds for comparison c-a, d-a, e-a.
...
Since we are analysing a spike-in study, we know the ground truth.
That is, we know that only the spike-in proteins (ecoli) are differentially expressed.
We can therefore evaluate the performance of the method, i.e. we assess
...
Instead of only calculating these quantities for the protein list that is returned for the
...
tprFdpHurdle[[i]] <- tprFdp(rowData(pe[["protein"]])[[paste0("hurdle_",i)]]$fisherPval,
