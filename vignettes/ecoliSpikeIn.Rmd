...
grid.arrange(grobs = volcano[1:9], nrow = 3)
```
...
grid.arrange(grobs = volcanoMed[1:9], nrow = 3)
```
...
grid.arrange(grobs = volcanoRidge, nrow = 3)
```
...
grid.arrange(grobs = tprFdpPlotRidge[1:9], ncol = 3)
```

- The median summarisation is vastly outperformed by robust summarisaton.
...
Note, that we have to redo the FDR correction. Indeed, the FDR is a set
...
grid.arrange(grobs = tprFdpRlmOptPlot[1:9], ncol = 3)
```
