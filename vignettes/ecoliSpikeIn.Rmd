...
is modelled using the parameter 'conditionb'. Hence, we assess the contrast
...
DE proteins are indeed close to the spiked in fold change of `r log2(concentrations["b"]/concentrations["a"]) %>% round(.,2)`.
...
tprFdpPlotBa <- tprFdpCompBa %>%
  ...
  geom_point(data = tprFdpCompBa[sum(tprFdpCompBa$adjPval < 0.01, na.rm=TRUE), ], aes(x = fdp, y = tpr), cex = 2)
tprFdpPlotBa
...
the low spike-in concentration because the ecoli proteins are mainly missing for these conditions.
